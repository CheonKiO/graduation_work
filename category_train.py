...
# 하이퍼파라미터
batch_size = 128
num_epochs = 20  # 원하는 추가 학습 epoch 수
...
num_upper_categories = len(dataset.upper_category_mapping)
num_categories = len(dataset.category_mapping)
...
# 모델 초기화 및 설정
model = UpperCategoryCategoryModel(num_upper_categories, num_categories).to(device)
model.load_state_dict(torch.load('upper_category_category_model10.pth', map_location=torch.device('cpu')))
model.to(device)

# 손실 함수 및 옵티마이저
criterion_upper_category = nn.CrossEntropyLoss()
...
def validate_model(model, dataloader):
    ...
    with torch.no_grad():
        for images, labels in tqdm(dataloader, desc="Validation"):
            ...
            # 라벨 처리
            upper_category_labels = labels['upper_category'].to(device)
            ...
            # 손실 계산
            loss_upper_category = criterion_upper_category(outputs['upper_category'], upper_category_labels)
            loss_category = criterion_category(outputs['category'], category_labels)
            
            # 총 손실
            total_loss = (loss_upper_category + loss_category)
            val_loss += total_loss.item()
            
            # 정확도 계산
            _, pred_upper_category = torch.max(outputs['upper_category'], 1)
            _, pred_category = torch.max(outputs['category'], 1)
            
            correct_upper_category += (pred_upper_category == upper_category_labels).sum().item()
            correct_category += (pred_category == category_labels).sum().item()
            total_samples += upper_category_labels.size(0)
        
    val_loss /= len(dataloader)
    val_upper_category_acc = correct_upper_category / total_samples
    val_category_acc = correct_category / total_samples
    
    print(f"Validation Loss: {val_loss:.4f}, Upper Category Acc: {val_upper_category_acc:.4f}, "
          f"Category Acc: {val_category_acc:.4f}")
    
    model.train()

# 학습 함수 수정
def train_model(model, dataloader, criterion_upper_category, criterion_category, optimizer, num_epochs=10):
    model.train()
    ...
    for epoch in range(num_epochs):
        ...
        for images, labels in tqdm(dataloader, desc=f"Epoch {epoch + 1}/{num_epochs}"):
            ...
            # 라벨을 텐서로 변환
            upper_category_labels = labels['upper_category'].clone().detach().to(device) if isinstance(labels['upper_category'], torch.Tensor) else torch.tensor(labels['upper_category'], dtype=torch.long).to(device)
        ...
        upper_category_acc = correct_upper_category / total_samples
        category_acc = correct_category / total_samples
        
        print(f"Epoch [{epoch+1}/{num_epochs}], Loss: {epoch_loss:.4f}, "
              f"Upper Category Acc: {upper_category_acc:.4f}, Category Acc: {category_acc:.4f}")
        validate_model(model, valid_loader)
        if (epoch+1)%5 == 0:
            # 학습이 완료된 모델 저장
            torch.save(model.state_dict(), f'upper_category_category_model{11 + epoch//5}.pth')
            print(f'모델 저장 완료: upper_category_category_model{11 + epoch//5}.pth')

# 학습 시작
train_model(model, train_loader, criterion_upper_category, criterion_category, optimizer, num_epochs)

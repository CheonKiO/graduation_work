...
# 모델 초기화 및 설정
model = UppermaterialModel(num_categories).to(device)
model.load_state_dict(torch.load('material_model12.pth', map_location=torch.device('cpu')))
model.to(device)
...
def validate_model(model, dataloader):
    ...
    with torch.no_grad():
        for images, labels in tqdm(dataloader, desc="Validation"):
            ...
            total_samples += material_labels.size(0)
...
# 학습 함수 수정
def train_model(model, dataloader, criterion_material, optimizer, num_epochs=10):
    model.train()
    ...
    for epoch in range(num_epochs):
        ...
        print(f"Epoch [{epoch+1}/{num_epochs}], Loss: {epoch_loss:.4f}, "
              f"material Acc: {material_acc:.4f}")
        validate_model(model, valid_loader)
        if (epoch+1)%5 == 0:
            # 학습이 완료된 모델 저장
            torch.save(model.state_dict(), f'material_model{15 + epoch//5}.pth')
            print(f'모델 저장 완료: material_model{15 + epoch//5}.pth')

# 학습 시작
train_model(model, train_loader, criterion_material, optimizer, num_epochs)

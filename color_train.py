...
# 하이퍼파라미터
batch_size = 128
num_epochs = 10  # 원하는 추가 학습 epoch 수
...
num_colors = len(dataset.color_mapping)
...
# 모델 초기화 및 설정
model = ColorModel(num_colors).to(device)
model.load_state_dict(torch.load('color_model11.pth', map_location=torch.device('cpu')))
model.to(device)
...
def validate_model(model, dataloader):
    ...
    with torch.no_grad():
        for images, labels in tqdm(dataloader, desc="Validation"):
            ...
            # 손실 계산
            loss_color = criterion_color(outputs['color'], color_labels)
            ...
            # 정확도 계산
            _, pred_color = torch.max(outputs['color'], 1)
            
            correct_color += (pred_color == color_labels).sum().item()
            total_samples += color_labels.size(0)
        
    val_loss /= len(dataloader)
    val_color_acc = correct_color / total_samples
    
    print(f"Validation Loss: {val_loss:.4f}, Color Acc: {val_color_acc:.4f}")
    
    model.train()

# 학습 함수 수정
def train_model(model, dataloader, criterion_color, optimizer, num_epochs=10):
    model.train()
    ...
    for epoch in range(num_epochs):
        ...
        for images, labels in tqdm(dataloader, desc=f"Epoch {epoch + 1}/{num_epochs}"):
            ...
            # 라벨을 텐서로 변환
            color_labels = labels['color'].clone().detach().to(device) if isinstance(labels['color'], torch.Tensor) else torch.tensor(labels['color'], dtype=torch.long).to(device)
        ...
        print(f"Epoch [{epoch+1}/{num_epochs}], Loss: {epoch_loss:.4f}, "
              f"Color Acc: {color_acc:.4f}")
        validate_model(model, valid_loader)
        if (epoch+1)%5 == 0:
            # 학습이 완료된 모델 저장
            torch.save(model.state_dict(), f'color_model{12 + epoch//5}.pth')
            print(f'모델 저장 완료: color_model{12 + epoch//5}.pth')

# 학습 시작
train_model(model, train_loader, criterion_color, optimizer, num_epochs)
